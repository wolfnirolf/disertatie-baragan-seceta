...
# 1. Încarcă fișierul
df = pd.read_excel("Precip-medii-pe-anotimp_1961-2020.xlsx")
df.columns = df.columns.str.strip()
df = df.set_index("An")

# 2. Setup figură
fig, axs = plt.subplots(2, 2, figsize=(14, 10))
fig.suptitle("Evoluția precipitațiilor sezoniere și trendurile pe termen lung – Câmpia Bărăganului (1961–2020)", fontsize=14)
plt.subplots_adjust(wspace=0.15)  # ✅ mai compact pe orizontală

# 3. Configuri
sezoane    = ["Iarna", "Primăvara", "Vara", "Toamna"]
culori     = ["#274472", "#1B5E20", "#7B1FE0", "#4E342E"]
pozitii    = {(0, 0): "Iarna", (0, 1): "Primăvara", (1, 0): "Vara", (1, 1): "Toamna"}
xticks_maj = [1961, 1965, 1970, 1975, 1980, 1985, 1990, 1995,
...
for (i, j), sezon in pozitii.items():
    ...
    # Precipitație medie
    ax.plot(x, y, lw=1.8, color=col, label="Nivel precipitații")
    ...
    # Linii verticale gri discrete
    for year in xticks_maj:
        ax.vlines(year, ymin=y_base, ymax=y_top, color="#dddddd", linestyle='-', linewidth=0.8, zorder=0)

    # Linii orizontale punctate personalizate (fără 0)
    y_start = start_levels.get(sezon, 20)
    for yline in range(y_start, int(np.ceil(y_top)), 10):
        if yline != 0:
            ax.hlines(yline, xmin=x_base, xmax=x_top, color="#dddddd", linestyle='--', linewidth=0.8, zorder=0)

    # Mini-segmente între ani
    for yr in xticks_min:
        ax.plot([yr, yr], [-0.02, 0.02], transform=ax.get_xaxis_transform(),
                color="gray", lw=0.5)

    # Text și linie pentru 2014
    if 2014 in x:
        ...
        ax.vlines(2014, ymin=y_text_top + pad, ymax=y_top,
                  linestyle="--", linewidth=1, color="gray")

        if not pd.isna(y.loc[2014]):
            ax.hlines(y=y.loc[2014], xmin=2013.5, xmax=2014.5,
                      linestyle=':', color='gray', linewidth=1)

    # Axe & legendă
    ax.set_xticks(xticks_maj)
    ax.set_xticklabels(xticks_maj, rotation=45)
    ax.set_ylabel("mm/lună")
    ax.set_title(sezon)
    ax.legend(loc="upper center", fontsize=9)  # ✅ legendă în colțul stânga sus



# 5. Finalizare
plt.tight_layout(rect=(0, 0.03, 1, 0.95))
plt.savefig("precipitatii_sezoniere_1961-2020.png", dpi=300)
plt.show()
