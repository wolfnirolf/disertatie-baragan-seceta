#!/usr/bin/env python3
"""
Generate a Walter-Lieth climate diagram.

Usage
-----
python walter_lieth_1961_1990.py [-i EXCEL] [-o FIGURE]
"""
...
def build_diagram(df: pd.DataFrame) -> plt.Figure:
    ...
    # --- Axis limits (updated) -------------------------------------------------
    temp_min_lim, temp_max_lim = -10, 35           ### MOD – urcă la 35 °C
    frac_0 = (0 - temp_min_lim) / (temp_max_lim - temp_min_lim)

    precip_max_lim = 100                           ### MOD – urcă la 100 mm
    ...
    ax_p.set_yticklabels([str(t) for t in ax_p.get_yticks()])
    ...
    ax_t.legend(h_p + h_t, l_p + l_t, title="Legendă", loc="upper left", frameon=True)

    fig.suptitle("Diagrama Walter-Lieth (1961-1990)", fontsize=14, fontweight="bold")
...
def main():
    ...
    ap.add_argument("-i", "--input", default="Date-climatologice-1961-1990.xlsx",
                    help="Excel file with climatological data (default: %(default)s)")
    ap.add_argument("-o", "--output",
                    help="Output image file (png, svg, pdf …). If omitted, shows the figure.")
    args = ap.parse_args()

    if not os.path.isfile(args.input):
        sys.exit(f"Nu a fost găsit fișierul: {args.input}")
